...
public abstract class ListGridFragment<T>
    ...
    protected DatasourceAdapter<T> createAdapter() {
        ...
        if (datasource instanceof Pagination) {
            datasourceAdapter = new PaginationAwareAdapter<T>(getActivity(), getItemLayout(), datasource) {
                @Override
            ...
            datasourceAdapter = new DatasourceAdapter<T>(getActivity(), getItemLayout(), datasource) {
